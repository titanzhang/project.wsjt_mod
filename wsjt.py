#!/usr/bin/env python
#----------------------------------------------------------------------- WSJT
# $Date: 2013-04-03 00:41:37 +0800 (Wed, 03 Apr 2013) $ $Revision: 3122 $
#
from Tkinter import *
...
root = Tk()
Version="9.5 r" + "$Rev: 3122 $"[6:-1]
print "******************************************************************"
print "WSJT Version " + Version + ", by K1JT"
print "Revision date: " + \
      "$Date: 2013-04-03 00:41:37 +0800 (Wed, 03 Apr 2013) $"[7:-1]
print "Run date:   " + time.asctime(time.gmtime()) + " UTC"

Title="  WSJT 9.5    r" + "$Rev: 3122 $"[6:-1] + "     by K1JT"
...
def dbl_click_text(event):
    ...
    if mode.get()=='Diana':
        report.delete(0,END)
        report.insert(0,'OOO')
    if mode.get()[:3]=='JT4' or mode.get()[:4]=='JT65':
...
def dbl_click3_text(event):
    ...
    if mode.get()[:4]=='JT65' or mode.get()[:3]=='JT4':
        ...
        # set DF according to the report
        tt=t1[n+21:]
        tt=tt.strip()
        n=tt.find(" ")
        tt=tt[0:n]
        if tt!='':
            Audio.gcom2.mousedf = int(tt)
#             itol=2
...
def toggletxdf(event=NONE):
    ...
    if ltxdf:
        ...
        btxdf.configure(text=t,bg='#ff0000',relief=SUNKEN)
    else:
        Audio.gcom2.ntxdf=0
        t="TxDF %4d" % 0
...
##        return
    ...
    if mode.get()=="FSK441" or mode.get()[:5]=="ISCAT" or \
       mode.get()=='JTMS' or mode.get()=='Diana' or mode.get()[:3]=='JT4':
        ...
        stdPrefix=(ToRadio.get() + " "+options.MyCall.get()).upper()
        t0=stdPrefix
        Audio.gcom2.t0msg=(t0+' '*22)[:22]
        nplain,naddon,ndiff=Audio.chkt0()
        ...
        tx2.insert(0,stdPrefix+" OOO")
        tx3.insert(0,stdPrefix+" RO")
        tx4.insert(0,stdPrefix+" RRR")
        tx5.insert(0,stdPrefix+" 73")
...
txfirst=Checkbutton(f5b2,text=' Tx Even',justify=LEFT,variable=TxFirst,width=12)
...
labreport=Label(f5b2,text='Rpt:',width=6,underline=0)
labreport.grid(column=0,row=1,sticky='E',padx=0)

btxdf=Button(f5b2,text='TxDF    0',command=toggletxdf,padx=1,pady=1,width=12)
btxdf.grid(column=1,row=0,sticky='EW',ipadx=0)

f5b3=Frame(f5b,bd=2,relief=GROOVE)
f5b3.grid(column=0,row=2,padx=2,sticky='EW')
genmsg=Button(f5b3,text=' Gen Msgs ',underline=1,command=GenStdMsgs,width=12,
            padx=1,pady=2)
genmsg.grid(column=0,row=0,sticky='W',padx=2)
auto=Button(f5b3,text='Auto is Off',underline=0,command=toggleauto,width=12,
            padx=2,pady=2)
auto.focus_set()
...
b1=Button(f5c, text='Tx1',underline=2,command=btx1,padx=1,pady=1,width=6)
...
b2=Button(f5c, text='Tx2',underline=2,command=btx2,padx=1,pady=1,width=6)
...
b3=Button(f5c, text='Tx3',underline=2,command=btx3,padx=1,pady=1,width=6)
...
b4=Button(f5c, text='Tx4',underline=2,command=btx4,padx=1,pady=1,width=6)
...
b5=Button(f5c, text='Tx5',underline=2,command=btx5,padx=1,pady=1,width=6)
...
b6=Button(f5c, text='Tx6',underline=2,command=btx6,padx=1,pady=1,width=6)
